...
public class MessageAPI extends BaseAPI{
	...
	public static TemplateMessageResult messageTemplateSend(String access_token,TemplateMessage templateMessage){
		...
		logger.info("Send tpl messag :"+messageJson);
		HttpUriRequest httpUriRequest = RequestBuilder.post()
				.setHeader(jsonHeader)
	 ...
	 * 根据模板的短ID获取长id
	 */
	public static TemplateMessageResult addTemplate(String access_token,String templateMessageShortId) {
		HttpUriRequest httpUriRequest = RequestBuilder.post()
				.setHeader(jsonHeader)
				.setUri(BASE_URI+"/cgi-bin/template/api_add_template")
				.addParameter("access_token", access_token)
				.setEntity(new StringEntity("{\"template_id_short\":\""+templateMessageShortId+"\"}",Charset.forName("utf-8")))
				.build();
		return LocalHttpClient.executeJsonResult(httpUriRequest,TemplateMessageResult.class);
	}
